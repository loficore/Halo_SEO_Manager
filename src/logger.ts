...
const logFormat = winston.format.combine(
    ...
    winston.format.printf((info: any) => { // 将 info 强制转换为 any
        // 优先从 info.module 获取，如果不存在则从 info.metadata?.module 获取
        const actualModuleName = info.module || info.metadata?.module || Modules.App;
...
const createModuleFilter = (moduleName: ModuleKey) => winston.format((info: any) => {
...
// 为每个模块创建日志文件传输器
const createDailyRotateFileTransport = (moduleName: ModuleKey, level: keyof typeof levels = 'debug') => {
...
// 导出日志接口
export const log = (level: keyof typeof levels, moduleName: ModuleKey, message: string, meta: Record<string, any> = {}) => {
    ...
    oldLogFiles.forEach(file => {
        if (fs.existsSync(file)) {
            try {
                ...
                logger.error(`Failed to remove old log file: ${file}`, { module: Modules.App, error: err });
            }
        }
    });

    logger.info('Logger initialized with Winston. Old log files cleaned.', { module: Modules.App });
};
