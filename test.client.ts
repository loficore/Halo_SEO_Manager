...
async function runTest() {
  ...
  if (!haloBaseUrl || !haloApiToken || !openaiApiKey || !openaiBaseUrl || !openaiModelName) {
  ...
  try {
    ...
    const selectedPost = allPosts.find(post => post.post.metadata.name === targetArticleId);

    if (!selectedPost) {
        log('error', Modules.App, `Failed to find article with ID: ${targetArticleId}`);
        return;
    }
    ...
    if (!articleData) {
        log('error', Modules.App, 'Failed to extract article data from the selected post.');
        return;
    }
    
    const articleId = articleData.article_id;
    log('info', Modules.App, `Selected article for optimization: "${articleData.title}" (ID: ${articleId})`);

    // 4. Execute optimization
    log('info', Modules.App, `Optimizing article ID: ${articleId}...`);
    const seoMeta = await seoOptimizer.optimizeArticle(articleId, articleData.content);
    ...
    if (!valid) {
        log('error', Modules.App, `SEO meta validation failed: ${errors.map(e => e.message).join(', ')}`);
        return;
    }
    ...
    log('info', Modules.App, `Attempting to publish SEO meta for article ID: ${articleId}...`);
    const publishSuccess = await seoPublisher.publishSeoMeta(selectedPost.post, seoMeta);

    if (publishSuccess) {
      log('info', Modules.App, 'Test finished successfully: SEO meta published.');
    } else {
